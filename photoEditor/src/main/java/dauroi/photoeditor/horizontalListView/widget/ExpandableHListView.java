 ...
 * differs from the {@link dauroi.photoeditor.horizontalListView.widget.HListView} by
 * allowing two levels: groups which can individually be expanded to show its
 * children. The items come from the
 ...
 * {@link android.widget.SimpleCursorTreeAdapter}) contain the preferred
 * position information for indicators.
 * <p/>
...
public class ExpandableHListView extends HListView {
	...
	public ExpandableHListView(Context context, AttributeSet attrs, int defStyle) {
		...
		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.ExpandableHListView, defStyle, 0);

		setGroupIndicator(a
				.getDrawable(R.styleable.ExpandableHListView_hlv_groupIndicator));
		setChildIndicator(a
				.getDrawable(R.styleable.ExpandableHListView_hlv_childIndicator));

		mIndicatorLeft = a.getDimensionPixelSize(
				R.styleable.ExpandableHListView_hlv_indicatorPaddingLeft, 0);
		mIndicatorTop = a.getDimensionPixelSize(
				R.styleable.ExpandableHListView_hlv_indicatorPaddingTop, 0);

		mIndicatorGravity = a.getInt(
				R.styleable.ExpandableHListView_hlv_indicatorGravity,
				Gravity.NO_GRAVITY);
		mChildIndicatorGravity = a.getInt(
				R.styleable.ExpandableHListView_hlv_childIndicatorGravity,
				Gravity.NO_GRAVITY);

		mChildIndicatorLeft = a.getDimensionPixelSize(
				R.styleable.ExpandableHListView_hlv_childIndicatorPaddingLeft,
				0);
		mChildIndicatorTop = a
				.getDimensionPixelSize(
						R.styleable.ExpandableHListView_hlv_childIndicatorPaddingTop,
						0);

		mChildDivider = a
				.getDrawable(R.styleable.ExpandableHListView_hlv_childDivider);
		a.recycle();
	}
	...
	/**
	 * Resolve start/end indicator. start/end indicator always takes precedence
	 * over left/right indicator when defined.
	 */
	...
	/**
	 * Resolve start/end child indicator. start/end child indicator always takes
	 * precedence over left/right child indicator when defined.
	 */
		...
		for (int i = 0, childFlPos = mFirstPosition - headerViewsCount; i < childCount; i++, childFlPos++) {
			...
			if (pos.position.type != lastItemType) {
				if (pos.position.type == ExpandableHListPosition.CHILD) {
					...
					indicatorRect.bottom = item.getBottom()
							+ mChildIndicatorTop;
				} else {
			...
			if (indicatorRect.top != indicatorRect.bottom) {
				...
				if (indicator != null) {
					...
					if (pos.position.type == ExpandableHListPosition.CHILD) {
						Gravity.apply(mChildIndicatorGravity,
								mChildIndicatorWidth, mChildIndicatorHeight,
								indicatorRect, mTempRect);
					} else {
						Gravity.apply(mIndicatorGravity, mGroupIndicatorWidth,
								mGroupIndicatorHeight, indicatorRect, mTempRect);
					}
	 ...
	 * 
	 * @param pos
	 *            The flat list position of the item whose indicator should be
	 *            returned.
	...
	private Drawable getIndicator(ExpandableHListConnector.PositionMetadata pos) {
		...
		if (pos.position.type == ExpandableHListPosition.GROUP) {
			...
			if (indicator != null && indicator.isStateful()) {
				...
				// has two states
				final int stateSet[] = pos.position.flatListPos == pos.groupMetadata.lastChildFlPos ? CHILD_LAST_STATE_SET
						: EMPTY_STATE_SET;
	 ...
	 * height, the height set by {@link #setDividerWidth(int)}.
	 * 
	 * @param childDivider
	 *            The drawable to use.
	 */
	...
	void drawDivider(Canvas canvas, Rect bounds, int childIndex) {
		...
		if (flatListPosition >= 0) {
			...
			ExpandableHListConnector.PositionMetadata pos = mConnector
					.getUnflattenedPos(adjustedPosition);
			// If this item is a child, or it is a non-empty group that is
			// expanded
			if ((pos.position.type == ExpandableHListPosition.CHILD)
					|| (pos.isExpanded() && pos.groupMetadata.lastChildFlPos != pos.groupMetadata.flPos)) {
	...
	boolean handleItemClick(View v, int position, long id) {
		final ExpandableHListConnector.PositionMetadata posMetadata = mConnector
		...
		if (posMetadata.position.type == ExpandableHListPosition.GROUP) {
			...
			if (posMetadata.isExpanded()) {
				...
				if (mOnGroupExpandListener != null) {
					mOnGroupExpandListener
							.onGroupExpand(posMetadata.position.groupPos);
				}
			...
			if (mOnChildClickListener != null) {
				playSoundEffect(SoundEffectConstants.CLICK);
				return mOnChildClickListener.onChildClick(this, v,
	...
	public boolean expandGroup(int groupPos, boolean animate) {
		ExpandableHListPosition elGroupPos = ExpandableHListPosition.obtain(
				ExpandableHListPosition.GROUP, groupPos, -1, -1);
		ExpandableHListConnector.PositionMetadata pm = mConnector
				.getFlattenedPos(elGroupPos);
		elGroupPos.recycle();
	...
	public interface OnGroupClickListener {
		 ...
		 */
		boolean onGroupClick(ExpandableHListView parent, View v,
	...
	public interface OnChildClickListener {
		 ...
		 *            clicked
		 * @param groupPosition
		 *            The group position that contains the child that was
		 *            clicked
		 * @param childPosition
		 ...
		 * @return True if the click was handled
		 */
		boolean onChildClick(ExpandableHListView parent, View v,
	...
	public long getSelectedId() {
		...
		if (getPackedPositionType(packedPos) == PACKED_POSITION_TYPE_GROUP) {
			...
			// It's a child
			return mAdapter.getChildId(groupPos,
	...
	public void setSelectedGroup(int groupPosition) {
		ExpandableHListPosition elGroupPos = ExpandableHListPosition
	...
	public boolean setSelectedChild(int groupPosition, int childPosition,
		...
		ExpandableHListPosition elChildPos = ExpandableHListPosition
				.obtainChildPosition(groupPosition, childPosition);
		ExpandableHListConnector.PositionMetadata flatChildPos = mConnector
	 ...
	 * {@link ExpandableHListView} method can either be a child or group. The
	 * two positions are packed into a single long which can be unpacked using
	...
	@Override
	ContextMenuInfo createContextMenuInfo(View view, int flatListPosition,
			long id) {
		if (isHeaderOrFooterPosition(flatListPosition)) {
	 ...
	 *            last child for a group, the state
	 ...
	 *            empty, the state {@link android.R.attr#state_empty} will be
	...
	/**
	 * Extra menu information specific to an {@link ExpandableHListView}
	 * provided to the
	 * {@link android.view.View.OnCreateContextMenuListener#onCreateContextMenu(ContextMenu, View, ContextMenuInfo) }
	 * callback when a context menu is brought up for this AdapterView.
	 */
	public static class ExpandableListContextMenuInfo implements
			ContextMenu.ContextMenuInfo {

		public ExpandableListContextMenuInfo(View targetView,
				long packedPosition, long id) {
		 ...
		 * one of the children Views of this {@link ExpandableHListView}.
		 */
	...
	static class SavedState extends BaseSavedState {
		 ...
		 * {@link ExpandableHListView#onSaveInstanceState()}
		 */
		SavedState(
		...
		private SavedState(Parcel in) {
			...
			in.readList(expandedGroupMetadataList,
					ExpandableHListConnector.class.getClassLoader());
		}
	...
	public void onRestoreInstanceState(Parcelable state) {
		...
		if (mConnector != null && ss.expandedGroupMetadataList != null) {
			mConnector
					.setExpandedGroupMetadataList(ss.expandedGroupMetadataList);
		}
	}
