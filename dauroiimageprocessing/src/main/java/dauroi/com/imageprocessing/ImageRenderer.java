...
@TargetApi(11)
public class ImageRenderer implements Renderer, PreviewCallback {
	public static final int NO_IMAGE = -1;
	public static final float CUBE[] = { -1.0f, -1.0f, 1.0f, -1.0f, -1.0f,
			1.0f, 1.0f, 1.0f, };
	...
	private float mBgRed = 0, mBgGreen = 0, mBgBlue = 0, mBgAlpha = 1;
	private boolean mEnableBlend = true;

	public ImageRenderer(final ImageFilter filter) {
		mFilter = filter;
	...
	public void onSurfaceCreated(final GL10 unused, final EGLConfig config) {
		GLES20.glClearColor(mBgRed, mBgGreen, mBgBlue, mBgAlpha);
	...
	public void onPreviewFrame(final byte[] data, final Camera camera) {
		...
		if (mRunOnDraw.isEmpty()) {
			runOnDraw(new Runnable() {
				@Override
				public void run() {
					ImageUtils.YUVtoRBGA(data, previewSize.width,
	...
	/**
	 * This method will recycle bitmap after using it.
	 * 
	 * @param bitmap
	 */
	...
	private void adjustImageScaling() {
		...
		if (mScaleType == ImageProcessor.ScaleType.CENTER_CROP) {
