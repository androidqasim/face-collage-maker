 ...
 */
public class SobelEdgeDetection extends ImageFilterGroup {
    ...
    private static List<ImageFilter> createFilters() {
        List<ImageFilter> filters = new ArrayList<ImageFilter>(2);
        filters.add(new GrayscaleFilter());
        filters.add(new TextureSamplingFilter(SOBEL_EDGE_DETECTION));
        return filters;
    }

    public void setLineSize(final float size) {
        ((TextureSamplingFilter) getFilters().get(1)).setLineSize(size);
    }
}
